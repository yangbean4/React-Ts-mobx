...
export class Config2Store extends StoreExt {
    @observable
    ...
    @observable
    filters: IConfig2Store.SearchParams = {}
    ...
    @action
    changeFilter = (data: IConfig2Store.SearchParams = {}) => {
        this.filters = { ...data }
        this.changepage(1)
    }
    @action
    setTargetConfig = (config: IConfig2Store.IConfigTarget) => {
    ...
    getConfigsList = async () => {
        try {
            const res = await this.api.config.getConfigs({ page: this.page, pageSize: this.pageSize, ...this.filters, version: 2 })
            const fmtArr: IConfig2Store.IConfig[] = res.data.map(item => {
                const {
    ...
    createConfig = async (config: IConfig2Store.IConfig) => {
        return await this.api.config.createConfig(config)
    }

    modifyConfig = async (config: IConfig2Store.IConfig) => {
...
export default new Config2Store()
