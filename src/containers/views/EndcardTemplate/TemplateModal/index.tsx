  ...
  getUploadprops = (fun: Function, key: string, type = '.png, .jpg, .jpeg, .gif', size?: number, cb?: Function, libao?: boolean) => {
    ...
    return {
      ...
      customRequest: (data) => {
        ...
        if (typeof this.uploadProgress[key] !== 'undefined') {
          config.onUploadProgress = e => {
            runInAction('set_progeress', () => {
              this.uploadProgress[key] = e.loaded / e.total * 100 | 0;
            })
          }
        }
        fun(formData, config).then(res => {
          const data = res.data
          this.props.form.setFieldsValue({
            [key]: data.url
          })
              ...
              !urlName ? (<Upload {...templateProps}>
                {
                  urlName || (
                    <Button style={{
                      backgroundImage: `linear-gradient(to right,#E1E6E9 ${this.uploadProgress.template_url}%, #fff 0%)`
                    }}>
          ...
          <FormItem {...formItemLayout} label="Template Image">
            {getFieldDecorator('template_image', {
              ...
              !imageName ? (<Upload {...imageProps}>
                <Button style={{
                  backgroundImage: `linear-gradient(to right,#E1E6E9 ${this.uploadProgress.template_image}%, #fff 0%)`
                }}>
                  <Icon type="iconshangchuan1" /> Upload Image
                </Button>
                <sub style={{ fontSize: '12px', color: '#999', transform: 'scale(.9, .9)', display: 'inline-block' }}>300*167px/168*293px</sub>
              </Upload>) : (<div>
