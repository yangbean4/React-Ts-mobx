            ...
            async (err, values): Promise<any> => {
                if (!err) {
                    ...
                    try {
                        if (!isNaN(Number(id))) {
                            values.id = id;
                            await this.props.editCommit(values);
                        } else {
                            await this.props.commitData(values);
                        }
                    } catch (err) {
                        console.log(err)
                    }
                        ...
                        {getFieldDecorator('campaign', {
                            ...
                            >
                                {OptionListDb.campaign.map(c => (
                                    <Select.Option key={c.campaign_id} value={c.campaign_id ? c.campaign_id.toString() : c.campaign_id} disabled={c.change_status == 0 ? false : true}>
