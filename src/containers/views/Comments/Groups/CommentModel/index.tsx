...
@inject(
  (store: IStore): IProps => {
    const { commentGroupStore, routerStore, globalStore } = store
    const { comment, createComment, modifyComment, clearComment } = commentGroupStore
    const { setBreadcrumbArr } = globalStore
    return { clearComment, comment, routerStore, setBreadcrumbArr, createComment, modifyComment }
  }
)
@observer
class CommentModal extends ComponentExt<IProps & FormComponentProps> {
  ...
  @action
  resetBread = (comment: ICommentGroupStore.IGroup) => {
    let arr = [
      {
    ...
    arr.push({
      title: `Edit ${comment.group_name}`,
      path: `/comments/groups/edit/${comment.id}`
    })
  ...
  componentWillMount() {
    ...
    if ((!isNaN(Id) && (!comment.id || comment.id !== Id)) && !this.props.type) {
      ...
      this.resetBread(this.props.comment)
