...
const asynchronousComponents = {
    Server: loadComponent(() => import(/* webpackChunkName: "Basic1" */ './Basic')),
...
interface IStoreProps {
    ...
    changeFilter?: (params: IConfig2Store.SearchParams) => void
    routerStore?: RouterStore
    targetConfig?: IConfig2Store.IConfigTarget
    submitConfig?: (data) => Promise<any>
    setTargetConfig?: (con?: IConfig2Store.IConfigTarget) => void
    getSidebar?: () => Promise<any>
}

@inject(
    (store: IStore): IStoreProps => {
        const { routerStore, config2Store, authStore } = store
        const { getSidebar } = authStore
        const { getConfigsList, changeFilter, targetConfig, submitConfig, setTargetConfig } = config2Store
...
class ConfigModal extends ComponentExt<IStoreProps> {
    ...
    @observable
    private editData: IConfig2Store.IConfigTarget = {}

    @observable
    private addConfigGroup: IConfig2Store.IConfigTarget = { basic1: [] }
    ...
    get allList() {
        ...
        const server = toTree(this.addConfigGroup['server'])
        return {
            basic1,
            basic2,
            server,
            all: [].concat(basic1, basic2, server),
        }
    }
    ...
    onCancel = () => {
        if (this.activeIndex === 0) {
            this.props.routerStore.push('/config2')
        } else {
    ...
    onSubmit = async (value) => {
        ...
        if (this.useTargetConfig.config_deploy_id) {
            ...
            const res = await this.props.submitConfig({ ...this.editData, ...this.useTargetConfig, version: 2 })
            this.$message.success(res.message)
            this.props.getSidebar()
        }
    ...
    initDetail = async (id?) => {
        ...
        try {
            if (id) {
                ...
                const { server, basic1, basic2, pid, pop } = res.data
                editData = { server, basic1, basic2, pid, pop }
            }
