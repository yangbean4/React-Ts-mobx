...
@inject(
  (store: IStore): IStoreProps => {
    const { config2Store, templateStore } = store
    const { targetConfig } = config2Store
...
class Basic extends ComponentExt<IProps & FormComponentProps> {
  ...
  get useConfigList(): conItemTree {
    ...
    if (Array.isArray(list)) {
      list = list.map(item => {
        if (item.key === 'configversion' && item.value_type === '1') {
        ...
        <Form className="dropZone" {...layout}>
          ...
          <Button type="primary" disabled={this.hasEditing} loading={this.loading} className='submitBtn' onClick={this.submit}>Submit</Button>
          {
            ...
            this.showWork && BasicType ? <Button className="cancelBtn" onClick={this.toggleWork}>Cancel</Button>
              : BasicType !== "server" ? <Button className='cancelBtn' onClick={this.lastStep} loading={this.loading}>Last Step</Button> : null
          }
        </Form>
